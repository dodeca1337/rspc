...
impl<TCtx> ProcedureStore<TCtx> {
    ...
    pub fn append(&mut self, key: String, exec: Box<dyn Layer<TCtx>>, ty: ProcedureDataType) {
        #[allow(clippy::panic)]
        if key.is_empty() || key == "ws" || key.starts_with("rpc.") || key.starts_with("drspc.") {
            panic!(
                "drspc error: attempted to create {} operation named '{}', however this name is not allowed.",
                self.name,
                key
            );
        }
        ...
        if self.store.contains_key(&key) {
            panic!(
                "drspc error: {} operation already has resolver with name '{}'",
                self.name, key
            );
        }
